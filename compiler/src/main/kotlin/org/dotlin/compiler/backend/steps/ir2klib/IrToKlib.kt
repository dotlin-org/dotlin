...
fun writeToKlib(env: KotlinCoreEnvironment, config: CompilerConfiguration, irResult: IrResult, outputFile: File) {
    ...
    val metadataSerializer =
        org.jetbrains.kotlin.backend.common.serialization.metadata.KlibMetadataIncrementalSerializer(
    ...
    val files = env.getSourceFiles()

    val metadataHeader = metadataSerializer.serializeHeader(
        moduleDescriptor = irResult.module.descriptor,
        fragmentNames = files.map { it.name }.distinct().sorted(), // TODO: Compiled files?
        emptyPackages = emptyList()
    )

    val serializedMetadata = metadataSerializer.serializedMetadata(
        fragments = emptyMap(), // TODO
        header = metadataHeader.toByteArray()
    )
    ...
    buildKotlinLibrary(
        ...
        moduleName = irResult.module.name.asString(),
        nopack = false, // TODO
        perFile = false, // TODO
