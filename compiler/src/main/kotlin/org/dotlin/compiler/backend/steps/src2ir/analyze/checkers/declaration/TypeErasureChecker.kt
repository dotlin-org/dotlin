...
object TypeErasureChecker : DeclarationChecker {
    override fun check(
        ...
        for (typeParam in declaration.typeParameters) {
            val reifiedKeyword = typeParam.modifierList?.getModifier(KtTokens.REIFIED_KEYWORD) ?: continue

            context.trace.report(ErrorsDart.UNNECESSARY_REIFIED.on(reifiedKeyword))
        }
    }
}